...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        x_cat = torch.cat((x_flat, c), dim=1)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    resconstruction_loss = nn.functional.binary_cross_entropy(input=x_hat, target=x)
    kl_divergence = -0.5 * (1 + logvar - mu ** 2 - var).sum() / N

    loss = (resconstruction_loss + kl_divergence)
