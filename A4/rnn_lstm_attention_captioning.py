...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = torch.tanh(torch.matmul(prev_h, Wh) + torch.matmul(x, Wx) + b)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    s = torch.matmul(prev_h, Wh) + torch.matmul(x, Wx) + b
    cosh, sinh = torch.square(torch.cosh(s)), torch.square(torch.sinh(s))
    ds = (cosh - sinh) / cosh * dnext_h

    dx = torch.matmul(ds, Wx.T)
    dWx = torch.matmul(x.T, ds)
    dprev_h = torch.matmul(ds, Wh.T)
    dWh = torch.matmul(prev_h.T, ds)
    db = torch.sum(ds, axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      xt = x[:, t, :]
      cur_h, cac = rnn_step_forward(xt, cur_h, Wx, Wh, b)
      cache.append(cac)
      h[:, t, :] = cur_h
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    _, D = cache[0][0].shape  # shape of x

    cur_dh = torch.zeros((N, H), **to_double_cuda)
    ...
    for t in range(T):
      ...
      dh_sum = cur_dh + dh[:, -t-1, :]
      cur_dx, cur_dh, cur_dWx, cur_dWh, cur_db = rnn_step_backward(dh_sum, cache[-t-1])
...
class WordEmbedding(nn.Module):
  ...
  def forward(self, x):
      ...
      x_onehot = torch.arange(V, **to_double_cuda) == x[..., None] - 1
      x_onehot = x_onehot.type(torch.double)
      x_onehot = x_onehot.reshape((N * T, V))

      out = torch.matmul(x_onehot, self.W_embed)
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    loss = 0
    for t in range(T):
      loss += nn.functional.cross_entropy(x[:, t, :], y[:, t], ignore_index=ignore_index, reduction='sum')
