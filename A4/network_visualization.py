...
def compute_saliency_maps(X, y, model):
  ...
  loss = softmax_loss(scores, y).sum()
  loss.backward()

  grads = X.grad.data
  saliency = grads.abs().max(axis=1)[0]
...
def softmax_loss(scores, y):
  ...
  term1 = -scores[torch.arange(N), y]
  sum_j = torch.sum(torch.exp(scores), axis=1)
  term2 = torch.log(sum_j)
  loss = term1 + term2
  loss /= N 
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    ...
    loss = softmax_loss(scores, target_y).sum()
    loss.backward()

    g = X_adv.grad.data
    dX = learning_rate * g / g.pow(2).sum().sqrt()
