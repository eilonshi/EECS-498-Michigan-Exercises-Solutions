...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    x_pad = torch.zeros(N, C, H + 2 * pad, W + 2 * pad)
    x_pad[:, :, pad:-pad, pad:-pad] = x

    H_tag = 1 + (H + 2 * pad - FH) // stride
    W_tag = 1 + (W + 2 * pad - FW) // stride

    out = torch.zeros(N, F, H_tag, W_tag)
    ...
    for n in range(N):
      for f in range(F):
        for j in range(H_tag):
          for i in range(W_tag):
            j_s = j * stride
            i_s = i * stride
            conv = x_pad[n, :, j_s:j_s+FH, i_s:i_s+FW] * w[f]
            out[n, f, j, i] = conv.sum() + b[f]
  ...
  def backward(dout, cache):
    ...
    #         j_s = j * stride
    #         i_s = i * stride
    #         dx_pad[n, :, j_s:j_s+FH, i_s:i_s+FW] += w[f] * dout[n, f, j, i]
    #         dw[f] += x_pad[n, :, j_s:j_s+FH, i_s:i_s+FW] * dout[n, f, j, i]
    
    # # db = dout.sum(axis=(0, 2, 3))
    # # print('after: db', db.shape)
