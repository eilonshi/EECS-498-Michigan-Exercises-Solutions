...
def slice_indexing_practice(x):
  ...
  M = x.shape[0]
  N = x.shape[1]
  ind1 = [i for i in range(M) if i % 2 == 0]
  ind2 = [i for i in range(N) if i % 2 == 1]
  even_rows_odd_cols = x[ind1, :]
  even_rows_odd_cols = even_rows_odd_cols[:, ind2]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  out = (
...
def count_negative_entries(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  num_neg = torch.count_nonzero(x[x<0]).item()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_neg
